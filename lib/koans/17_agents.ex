defmodule Agents do
  ...
  koan "Agents maintain state, so you can ask them about it" do
    ...
    assert Agent.get(pid, &(&1)) == "Hi there"
    assert Agent.get(pid, fn a -> a end) == "Hi there"
  end
  ...
  koan "Agents may also be named so that you don't have to keep the pid around" do
    ...
    assert is_atom(AgentSmith) === true
    assert AgentSmith === Elixir.AgentSmith
    assert Elixir.AgentSmith === :"Elixir.AgentSmith"
  end
  ...
  koan "Update to update the state (reuse atom?)" do
    Agent.start_link(fn -> "Hi there" end, name: AgentSmith)

    Agent.update(AgentSmith, fn old ->
      String.upcase(old)
    end)

    assert Agent.get(AgentSmith, & &1) == "HI THERE"
  end
